...
Функция: Авторизация подтвержденным пользователем
  ...
  Предыстория: :
    Пусть В системе заведен пользователь "admin3" с параметрами:
      | admin  | false |
      | status | 1     |
  ...
  Сценарий: Авторизация подтвержденным пользователем
    Если Авторизоваться пользователем "admin3"
    То Будет открыта страница "Домашняя странца"
    И На главной странице пользователя "admin2" элемент Вошли как имеет текст "Вошли как ${admin3 ->логин}"
